...
namespace GADE_Task_1
{
    ...
    {
        public MeleeUnit(int x, int y, int health, int speed, int attack, int attackRange, string team, string symbol)
        ...
        {
            if (DistanceTo(attacker) == attackRange)  //checking to see if the unit is in the range of an enemy
            {
                return true;
            }
            else
        ...
        public override Unit ClosestUnit(List<Unit> list)
        {
            ...
            foreach (Unit i in list)
            {
                if (i.Team != team) //if its not in the units team 
                {
                    ...
                    {
                        smallestRange = DistanceTo(i);  // sets 
        ...
        public class DeathException : System.Exception //deathException that derives from the System.exception 
        {
        ...
        public override bool IsDead()   // method to check if the unit is died
        {
